...
public class SZBlocks {
	...
	private static BlockBehaviour.Properties rottenWool(MaterialColor materialColor) {
		return BlockBehaviour.Properties.of(Material.WOOL, materialColor).strength(0.8F).sound(SoundType.WOOL);
	}

	private static BlockBehaviour.Properties trophies() {
		return BlockBehaviour.Properties.of(Material.METAL, MaterialColor.METAL).requiresCorrectToolForDrops().strength(3.0F, 6.0F).sound(SoundType.METAL);
	}
}
